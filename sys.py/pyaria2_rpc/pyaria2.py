...
class Jsonrpc(object):
    ...
    def __init__(
        ...
        self.port = port
        self.serverUrl = \
            'http://{host}:{port}/jsonrpc'.format(**locals())
    ...
    def _post(
        ...
        paramsObject = self._genParams(action, params)
        print paramsObject
        resp = requests.post(self.serverUrl,
                             data=json.dumps(paramsObject))
        result = resp.json()
        if 'error' in result:
            return onFail(result['error']['code'], result['error'
                          ]['message'])
        else:
    ...
    def addUri(self, uris, options=None):
        ...
        if isinstance(uris, basestring):
            return self._post(Jsonrpc.ADDURI_METHOD, [[uris], options],
                              success)
        else:
            return self._post(Jsonrpc.ADDURI_METHOD, [uris, options],
                              success)
    ...
    def tellStatus(self, gid, keys=None):
        ...
        return self._post(Jsonrpc.TELLSTATUS_METHOD, [gid, keys],
                          success)
    ...
    def tellWaiting(
        ...
        return self._post(Jsonrpc.TELLWAITING_METHOD, [offset, num,
                          keys], success)
    ...
    def tellStopped(
        ...
        return self._post(Jsonrpc.TELLSTOPPED_METHOD, [offset, num,
                          keys], success)
    ...
    def purgeDownloadResult(self):
        ...
        return self._post(Jsonrpc.PURGEDOWNLOADRESULT_METHOD, [],
                          success)
    ...
    def removeDownloadResult(self, gid):
        ...
        return self._post(Jsonrpc.REMOVEDOWNLOADRESULT_METHOD, [gid],
                          success)
